...
class SaveManager:
    ...
    def detect_save_path(self):
        ...
        try:
            ...
            possible_paths.append(os.path.join(steam_path, "steamapps", "common", "Subnautica", "SNAppData", "SavedGames"))
        except WindowsError:
            logging.warning("Steam installation not found in registry")

        # Check Epic Games installation
        epic_path = os.path.expandvars(r"%PROGRAMDATA%\Epic\EpicGamesLauncher\Data\Manifests")
        if os.path.exists(epic_path):
            for file in os.listdir(epic_path):
                if file.endswith(".item"):
                    with open(os.path.join(epic_path, file), 'r') as f:
                        content = f.read()
                        if "Subnautica" in content:
                            # Extract installation directory from the manifest file
                            # This is a simplified approach and might need adjustment
                            install_location = content.split('"InstallLocation": "')[1].split('"')[0]
                            possible_paths.append(os.path.join(install_location, "SNAppData", "SavedGames"))
                            break
        ...
        possible_paths.extend([
            ...
            os.path.normpath(os.path.join(os.getenv('APPDATA'), '..', 'LocalLow', 'Unknown Worlds', 'Subnautica', 'Subnautica', 'SavedGames'))
        ])

        for path in possible_paths:
            if os.path.exists(path):
                logging.info(f"Detected Subnautica save path: {path}")
                return path
